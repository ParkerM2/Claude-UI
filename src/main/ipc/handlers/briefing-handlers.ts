...
export function registerBriefingHandlers(router: IpcRouter, service: BriefingService): void {
  router.handle('briefing.getDaily', () =>
    Promise.resolve(service.getDailyBriefing()),
  );

  router.handle('briefing.generate', async () =>
    await service.generateBriefing(),
  );

  router.handle('briefing.getConfig', () =>
  ...
  router.handle('briefing.getSuggestions', () =>
    Promise.resolve(service.getSuggestions()),
  );
}
